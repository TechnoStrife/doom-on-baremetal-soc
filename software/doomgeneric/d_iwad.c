...
char *D_FindIWAD(int mask, GameMission_t *mission)
{
    ...
    else
    {
        ...
        /*
         * If no IWAD was found on the filesystem, try known IWAD names
         * as embedded/linked-in WADs (e.g., doom1.wad). This calls
         * W_OpenFile to probe backends (such as the embedded backend)
         * and allows running without a filesystem.
         */
        if (result == NULL)
        {
            for (i = 0; i < arrlen(iwads); ++i)
            {
                ...
                /* Probe the wad backends for this IWAD name. */
                wad_file_t *wf = W_OpenFile((char *)iwads[i].name);

                if (wf != NULL)
                {
                    /* Found embedded/linked IWAD. Close probe handle and return name. */
                    W_CloseFile(wf);
                    *mission = iwads[i].mission;
                    return strdup(iwads[i].name);
                }
            }
        }
    }
